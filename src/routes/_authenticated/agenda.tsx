...
function AgendaPage() {
  ...
  function updateDepositEntry(
    index: number,
    patch: Partial<DepositEntry>
  ) {
    setDepositEntries(
      (current) =>
        current.map(
  ...
  async function fetchDeposits(
    ...
    const {
      ...
      .from(
        "studio_appointment_deposits"
      )
      .select(
        "id, appointment_id, amount, received_date"
      )
      .eq(
        "appointment_id",
        appointmentId
  ...
  async function saveAppointment() {
    ...
    try {
      ...
      if (appointmentId) {
        ...
        for (const entry of validDeposits) {
          const amount =
            parseMoney(
              entry.amount
            );
