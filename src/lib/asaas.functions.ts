...
/**
 * Cria um Checkout avulso (DETACHED) com PIX.
 * O acesso só é liberado pelo webhook, após a confirmação do pagamento.
 */
export const createAsaasPixCheckout = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { plan: SubscriptionPlan }) => {
    if (input?.plan !== "monthly" && input?.plan !== "yearly") {
      throw new Error("Plano inválido.");
  ...
  .handler(async ({ data, context }): Promise<CheckoutResult> => {
    ...
    if (!res.ok) {
      console.error("[Asaas] POST /checkouts PIX falhou", res.status, JSON.stringify(body));
      const description = (
        body as { errors?: Array<{ description?: string }> } | null
      )?.errors?.[0]?.description;
      throw new Error(description ?? `Asaas retornou o status ${res.status}.`);
    ...
    if (!checkoutId || !checkoutUrl) {
      console.error("[Asaas] resposta PIX sem link de checkout", JSON.stringify(body));
      throw new Error("O Asaas não retornou o link do checkout.");
    }

    const { error: updateError } = await supabase
      .from("subscriptions")
      .update({ plan: data.plan, asaas_checkout_id: checkoutId, payment_method: "pix" })
      .eq("user_id", userId);
    if (updateError) throw updateError;
