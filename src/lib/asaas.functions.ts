...
/**
 * Cria um Checkout avulso (DETACHED) hospedado pelo Asaas, com PIX e cartão
 * disponíveis na mesma página. A cliente preenche os próprios dados no Asaas.
 * A API Key nunca sai do servidor e o acesso só é liberado pelo webhook.
 */
export const createAsaasCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<CheckoutResult> => {
    ...
    if (!res.ok) {
      console.error("[Asaas] POST /checkouts falhou", res.status, JSON.stringify(body));
      const description = (
